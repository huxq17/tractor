# tractor
轻量级的任务管理器。 dev分支通过对okio源码进行修改来提升了下载的速度,有下载速度方面要求的可以clone dev分支的代码，jcenter上提供的是dev分支的依赖
###Usage
----
...
```groovy
dependencies {
   compile 'com.huxq17.android:tractor:1.0.2'
}
```
...
在平时的android开发工作中，我们经常需要执行耗时操作，有时为了用户体验还需要显示个等待框，我之前的做法都是开一个线程，然后用handler发消息进行显示和关闭等待框以及相关的ui操作。如果任务比较多的话,频繁的new Thread会让代码看上去比较混乱，而且还不好管理，针对这种情况我写了tractor。tractor主要的作用有：
 ...
 3. 支持大文件上传，多线程断点下载，get,post以及其他的网络请求
